...
namespace DarkcupGames
{
	...
	private static string getProductName()
	{
        ...
        private static void computeDeviceUniqueID()
        {
            string systemID = "";
#if UNITY_EDITOR && UNITY_IPHONE
		systemID = getProductName()+"IOS_"+SystemInfo.deviceUniqueIdentifier;
#elif UNITY_EDITOR && UNITY_ANDROID
            //temporary code to remove the 'phonecall' permission on android
            //systemID = getProductName()+"ANR_"+GameManager.Instance.DeviceID;
            systemID = getProductName() + "ANR_" + "THIS IS SYSTEMID";//fix for errr Huy 22/09/2020
#elif UNITY_IPHONE
